...
- Merged **Peacock: Enter a Color** with the visual picker into a single **Peacock: Choose a Custom Color** command: running it with no arguments now opens the [Custom Color Picker](../guide/README.md#custom-color-picker) directly, and its color field accepts every format the old input box did (hex, named colors, and `rgb`/`rgba`/`hsl`/`hsla`/`hsv`/`hsva` values), not just hex. The `peacock.enterColor` command ID is unchanged, so existing keybindings and scripts that invoke it programmatically with a color argument keep working exactly as before ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Designed the color picker panel with a modern layout: a labeled color-well/field group, an **eyedropper button** (shown when the browser EyeDropper API is available) to sample any color from anywhere on screen, and a live **title bar contrast preview** — a sample swatch plus a WCAG contrast ratio badge — that reuses Peacock's own title-bar contrast logic, so a light custom color (e.g. orange or pink) is confirmed to get the same automatic dark-font contrast treatment as any other color, before you even apply it ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Clicking the Peacock color in the status bar now opens the [Custom Color Picker](../guide/README.md#custom-color-picker) pre-filled with the color that's already showing, instead of only copying it to the clipboard. The clipboard-copy behavior hasn't gone away — it's still one click away via **Peacock: Copy the Current Color to the Clipboard** in the Command Palette — but the status bar itself is now a shortcut straight into editing the color you're looking at ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
...
- Added `peacock.affectAgentsWindow` setting — when enabled, Peacock colorizes VS Code's Agents Window (the dedicated top-level window for managing Copilot/Claude/Codex agent sessions, `src/vs/sessions/` upstream), which has its own title bar and panel chrome distinct from the standard editor window's tokens. Sets `agents.background`, which colors the window shell/title bar the same way `titleBar.activeBackground` does for the standard editor window. VS Code also registers `agents.background` as the *default* color for `inactiveSessionView.background` — the large session/composer view in the center of the window — so, left alone, accenting the title bar would also wash the accent color over that big content area. To prevent that, Peacock explicitly sets `inactiveSessionView.background`/`inactiveSessionView.foreground` to a neutral, theme-aware value (not the accent color), with the foreground computed for contrast the same way Peacock already computes the title bar's foreground, so the center pane stays visually neutral while only the title bar picks up the accent (mirroring the "chrome strips only" philosophy `titleBar.activeBackground` already follows elsewhere). Deliberately does **not** set `agentsPanel.*` (the chat/files/terminal card panels), letting those keep their own theme-correct defaults. Opt-in, defaults to `false`, and is a no-op on VS Code versions that predate these tokens. Because the Agents Window is a single window shared across all workspaces rather than tied to any one folder, Peacock also mirrors these keys to the user (global) settings in addition to its usual workspace-scoped write, so the Agents Window picks up the color regardless of which workspace last applied it; disabling the setting or unapplying/resetting the color clears the mirrored keys too (including stale `agentsPanel.background`/`agentsPanel.foreground` left over from an earlier version of this change)

### Fixes

- Fixed a script-injection vulnerability in the color picker: a color value that passes `isValidColorInput()` (tinycolor's `rgb()`/`hsl()` parsing isn't anchored to the whole string) could still contain characters that break out of the HTML attribute it's interpolated into -- e.g. a workspace's committed `peacock.color` set to `rgb(1,2,3)" autofocus onfocus="..."` would run arbitrary script in the picker webview the moment it opened. The starting color is now always normalized to a plain `#rrggbb[aa]` hex string before being embedded, and the panel's Content-Security-Policy uses a per-render nonce for its inline script instead of `'unsafe-inline'`, so no other inline script can run even if a future change reintroduces an unnormalized value ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Fixed highlighting **Custom color…** in the **Change to a Favorite Color** Quick Pick (before ever selecting it) unapplying every current Peacock color: the live-preview-on-highlight logic parsed the picker's own Quick Pick entry as if it were a favorite, got back invalid input, and responded by clearing the title bar/activity bar/status bar colors the instant the item was merely arrowed over ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Fixed the color picker resetting to the starting color (and losing its live contrast preview) if the panel was hidden and re-shown, e.g. by switching to another editor tab and back — `retainContextWhenHidden` is now enabled so the panel's own state survives being hidden
- Fixed the color picker's initial contrast preview computing a black-background pairing instead of the picker's own peacockGreen fallback when there's no `peacock.color` set yet, since the preview was computed from the raw (empty) starting color instead of the same resolved fallback the panel actually displays
- Fixed a same-color-picker race where closing the panel (the tab's own close button) while a preview message was still queued — e.g. from a fast color-well drag — could leave that stale preview applied after the revert, or throw an unhandled rejection from posting to the now-disposed webview. Closing the panel now reverts through the same serialized message queue as every other preview/apply/cancel, and every post to the webview is guarded against the panel already being disposed
- Fixed **Apply** silently doing nothing when a typed color looked plausible to the picker's own client-side heuristic (any run of letters, for named colors) but wasn't a real color Peacock recognizes (e.g. a typo like "purpel"). The host now tells the webview when a previewed/applied color is actually invalid, so the field is marked invalid and Apply is disabled instead of the click being silently dropped
- Removed an unused `wouldBeApplied` field from the color picker's contrast-preview computation that nothing ever read
- Fixed the color picker's Apply/Cancel buttons and live preview doing nothing at all: the panel's inline script contained a regex with `\s` and `\(`, which the _outer_ TypeScript template literal silently strips the backslash from (they aren't recognized string escapes), producing an unterminated-group regex and a `SyntaxError` that aborted the entire inline script before any event listener was ever attached. The color well, hex field, eyedropper, Apply, Cancel, and the title bar contrast preview all appeared in the panel but were completely inert. Fixed by properly escaping the backslashes so they survive the outer template literal ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Fixed a race condition in the color picker's webview message handling: rapid preview messages (e.g. from dragging the color well or using the eyedropper) were serialized through a queue that permanently wedged after any single handler call rejected, silently dropping every later message for the rest of that picker session. The queue is now decoupled from each call's own result, so one rejected preview/apply/cancel can no longer block the ones that follow it ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Fixed **Enter**/**Escape** in the color picker only working while the hex/color field itself had focus. Enter now applies (and Escape cancels) no matter which control has focus — the native color well, the hex field, or the eyedropper button — matching the Apply/Cancel buttons' own keyboard behavior ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Fixed the color picker's title bar contrast preview always assuming the title bar is colored with a dark foreground applied, even when `peacock.affectTitleBar` is off or `peacock.keepForegroundColor` is on — in either case Peacock's real apply logic never writes that foreground/background pairing, so the preview could show a passing contrast badge for a color that wouldn't actually get that treatment. The preview now reflects both live settings and shows a note under the badge explaining when it doesn't apply ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)

### Tests

- Added regression tests proving a crafted color value cannot break out of the picker's HTML (the interpolated value is always normalized hex), that the CSP's script-src carries a per-render nonce rather than `'unsafe-inline'`, that an 8-digit alpha hex color seeds the native color well with its alpha-less prefix, that highlighting **Custom color…** in the favorites Quick Pick leaves the current color untouched, that the initial contrast preview resolves to peacockGreen (not black) when there's no starting color, that closing the panel mid-drag reverts correctly without throwing, and that an invalid typed color disables Apply via the host's `{type: 'invalid'}` message ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Added a regression test that parses the color picker's generated inline `<script>` with Node's `vm` module and asserts it does not throw a `SyntaxError`, plus a test asserting the function-color regex's backslashes survive the outer template literal intact — together they catch the exact class of "silently-stripped-escape" bug that made every button and live preview in the panel inert ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Added unit and host-lane coverage proving the title bar contrast preview uses Peacock's real contrast logic for light custom colors (orange, pink, and a third representative light color), asserting the same dark foreground/WCAG-passing ratio pairing `applyColor()` itself would choose ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Added host-lane coverage for **Peacock: Choose a Custom Color** with no argument (apply-and-persist, cancel-reverts) and unit coverage for the panel's eyedropper button and contrast preview markup ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Added a regression test for the message-handler queue race condition, proving a rejected handler call no longer blocks subsequent queued messages ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
...
- Added a regression test asserting the status bar item's `command` is **Peacock: Choose a Custom Color**, so clicking it opens the picker rather than silently regressing back to only copying the color ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Added functional regression tests that actually execute the color picker's inline `<script>` against a minimal fake DOM (not just parse it) and simulate keydown events, proving Enter applies and Escape cancels regardless of which control has focus, that Enter is a no-op while Apply is disabled, and that Enter on a focused button (e.g. Cancel) doesn't also fire Apply ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Added unit and host-lane coverage proving the title bar contrast preview correctly flags itself as not applied when `peacock.affectTitleBar` is off or `peacock.keepForegroundColor` is on, and that the picker surfaces a note explaining why in those cases ([#755](https://github.com/johnpapa/vscode-peacock/pull/755) code-review follow-up)
- Added host-lane coverage for `peacock.affectAgentsWindow`: `agents.background` and the neutral `inactiveSessionView.*` counter-override are set/unset with the setting enabled/disabled, `inactiveSessionView.foreground` respects `peacock.keepForegroundColor`, dedicated light/dark theme cases confirm the neutral background/foreground is genuinely contrast-aware and independent of the accent color's own lightness, the colors are mirrored to (and cleared from) the user/global settings alongside the workspace settings, and a stale `agentsPanel.background` left over from an earlier version of this feature is cleared from both the workspace and user settings — mirroring the existing `windowBorder` test pattern in `affected-elements.test.ts`. Also added pure Vitest unit coverage for the new `getAgentsWindowNeutralStyle()` helper in `color-library.ts`
...
- Rewrote every hex/named/RGB/HSL/HSV `Enter a Color` format test to drive the merged **Choose a Custom Color** picker's `apply` message instead of the retired `InputBox` stub, since typed input now goes through the picker's own field rather than a separate input box. Removed the now-unused `createFakeInputBox()` test double, keeping `createFakeWebviewPanel()` as the single shared double in `src/test/suite/lib/` for `affected-elements.test.ts`, `save-starter-favorite-colors.test.ts`, `color-input.test.ts`, and `custom-color-picker.test.ts` ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
...
- Fixed a `tinycolor2` CJS/ESM interop bug: `color-library.ts`'s `import * as tinycolor from 'tinycolor2'` compiles to a raw `require()` under the project's classic (non-`esModuleInterop`) TypeScript/webpack build, but under Vite/esbuild's stricter ESM semantics a namespace import can never be callable, so any color-manipulation call built with Vite (including the new Vitest unit lane) failed with `TypeError: ... is not a function`. Switched to `import tinycolor = require('tinycolor2')`, which compiles identically (a plain `require()`) under both toolchains ([#670](https://github.com/johnpapa/vscode-peacock/issues/670))
...
- An earlier draft of this migration deleted several host-suite assertions (notably around `darken`/`lighten`, `affectedElements` toggles, `excludedSettings` reset behavior, and sidebar color removal) without an equivalent replacement, which would have reduced actual coverage. Restored that coverage as host tests — the unit lane only replaces host tests where the underlying logic is genuinely pure and the unit test exercises the same branches, not as a blanket substitution. Net result: 142 fast unit tests added, and host suite coverage is at or above its pre-migration baseline ([#670](https://github.com/johnpapa/vscode-peacock/issues/670)).
...
- Grouped Dependabot updates ([.github/dependabot.yml](.github/dependabot.yml)): npm minor/patch bumps into one PR, npm major bumps kept separate for extra scrutiny, and all GitHub Actions bumps grouped together — cuts down on the 11-separate-PRs triage burden.
