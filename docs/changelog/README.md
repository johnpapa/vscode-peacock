...
- Added `peacock.affectAgentsWindow` setting — when enabled, Peacock colorizes VS Code's Agents Window (the dedicated top-level window for managing Copilot/Claude/Codex agent sessions, `src/vs/sessions/` upstream), which has its own title bar and panel chrome distinct from the standard editor window's tokens. Sets `agentsPanel.background` (chat/files/terminal card panels) and a contrast-aware `agentsPanel.foreground` computed the same way Peacock already computes the title bar's foreground, so a light accent color still gets readable dark text. Deliberately does **not** set `agents.background`: VS Code registers it as the *default* color for other tokens too, including the title bar and the large session/composer view background, so setting it washes over big content areas Peacock otherwise never recolors (mirroring the "chrome strips only" philosophy `titleBar.activeBackground` already follows). Opt-in, defaults to `false`, and is a no-op on VS Code versions that predate these tokens. Because the Agents Window is a single window shared across all workspaces rather than tied to any one folder, Peacock also mirrors the `agentsPanel.*` keys to the user (global) settings in addition to its usual workspace-scoped write, so the Agents Window picks up the color regardless of which workspace last applied it; disabling the setting or unapplying/resetting the color clears the mirrored keys too (including a stale `agents.background` left over from an earlier version of this change)
...
- Added host-lane coverage for `peacock.affectAgentsWindow`: colors are set/unset with the setting enabled/disabled, `agentsPanel.foreground` respects `peacock.keepForegroundColor`, dedicated light/dark accent color cases confirm `agentsPanel.foreground` is genuinely contrast-aware (not just a fixed value), the colors are mirrored to (and cleared from) the user/global settings alongside the workspace settings, and a stale `agents.background` left over from an earlier version of this feature is cleared from both the workspace and user settings — mirroring the existing `windowBorder` test pattern in `affected-elements.test.ts`
