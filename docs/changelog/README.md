...
- Reverted the `peacock.affectFileTreeSelection` setting added in [#756](https://github.com/johnpapa/vscode-peacock/pull/756). That PR had been intentionally held as a draft pending community feedback on [#539](https://github.com/johnpapa/vscode-peacock/issues/539), and was merged before that feedback loop had run its course. Reverting to keep `main` matching the maintainer's actual decision; the implementation remains available in git history (commit `0ccb1d8`) to reopen once feedback settles on outline-vs-fill.
...
- Added a visual color picker to the **Peacock: Change to a Favorite Color** flow. The favorites Quick Pick now ends with a `$(paintcan) Custom color…` item that opens a webview with a native color well kept in sync with a hex/name/formula text field. Every edit previews live (the same way hovering a favorite already does, without unapplying your current color just from arrowing over the item), **Apply** keeps the color, and **Cancel** (or closing the panel) reverts to the color that was active before the picker opened. The Quick Pick now always opens, even with zero favorites saved, so the picker is reachable regardless ([#708](https://github.com/johnpapa/vscode-peacock/issues/708))
...
- Designed the color picker panel with a modern layout: a labeled color-well/field group, an **eyedropper button** (shown when the browser EyeDropper API is available) to sample any color from anywhere on screen, and a live **title bar contrast preview** — a sample swatch plus a WCAG contrast ratio badge — that reuses Peacock's own title-bar contrast logic, so a light custom color (e.g. orange or pink) is confirmed to get the same automatic dark-font contrast treatment as any other color, before you even apply it. The preview reflects your actual `peacock.affectTitleBar`/`peacock.keepForegroundColor` settings (with a note when the pairing doesn't apply), an invalid typed color (e.g. a color-name typo) marks the field invalid and disables **Apply** instead of doing nothing, **Enter**/**Escape** apply/cancel no matter which control has focus, and the panel's state (including the preview) survives being hidden and re-shown, e.g. by switching editor tabs ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
...
- Hardened the color picker webview: the starting color is always normalized to a plain `#rrggbb[aa]` hex string before being embedded in the panel's HTML, and its Content-Security-Policy uses a per-render nonce for the inline script instead of `'unsafe-inline'`, so an untrusted `peacock.color` value (e.g. from a shared workspace) can't break out of the markup or run injected script ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
...
- Ran a dedicated post-merge security audit of the color picker webview, re-confirming the `getColorHex()`-normalization + nonce-scoped CSP fix (above) fully closes the attribute/script-injection class of bug, that no CSS injection exists (the `<style>` block is fully static; runtime style writes are DOM property assignments fed by already-normalized hex, not string-built CSS), and that removing the hex text field would be a pure UX regression with no security benefit, since typed input never touches HTML interpolation. All 38 existing regression tests re-verified passing; no code changes were needed. See the write-up on [#755](https://github.com/johnpapa/vscode-peacock/pull/755#issuecomment-5704284977)
