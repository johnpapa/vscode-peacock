...
- Added host-lane coverage for `peacock.affectAgentsWindow`: colors are set/unset with the setting enabled/disabled, `agentsPanel.foreground` respects `peacock.keepForegroundColor`, and dedicated light/dark accent color cases confirm `agentsPanel.foreground` is genuinely contrast-aware (not just a fixed value) — mirroring the existing `windowBorder` test pattern in `affected-elements.test.ts`
