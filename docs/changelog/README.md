...
- Added `peacock.affectAgentsWindow` setting — when enabled, Peacock colorizes VS Code's Agents Window (the dedicated top-level window for managing Copilot/Claude/Codex agent sessions, `src/vs/sessions/` upstream), which has its own title bar and panel chrome distinct from the standard editor window's tokens. Sets `agentsPanel.border` (the card panel border color — `agentsCard.border`/`agentsBottomPanel.border` both default onto it, so this alone colors all three) and `agentsGradient.tintColor` (the shell's soft background gradient tint, rendered at low opacity only in the margins between floating panels). Deliberately does **not** set `agents.background`: VS Code ties that single token, unconditionally, to the title bar, the whole shell background, *and* — via a hardcoded rule in VS Code's own stylesheet — the left Sessions sidebar too, with no separate token available to color just the title bar. Also does not set `agentsPanel.background`/`agentsPanel.foreground` (which default onto `activeSessionView.background`, the large session/composer view in the center of the window). Leaving both unset keeps the sidebar and center session view visually neutral (mirroring the "chrome strips only" philosophy that keeps Peacock off `sideBar.background`/`editor.background` in the standard editor window) while still giving the Agents Window a visible, contained accent via its card borders and shell gradient tint. Opt-in, defaults to `false`, and is a no-op on VS Code versions that predate these tokens. Because the Agents Window is a single window shared across all workspaces rather than tied to any one folder, Peacock also mirrors these keys to the user (global) settings in addition to its usual workspace-scoped write, so the Agents Window picks up the color regardless of which workspace last applied it; disabling the setting or unapplying/resetting the color clears the mirrored keys too (including stale `agents.background`/`agentsPanel.background`/`inactiveSessionView.*` values left over from earlier iterations of this change)
...
- Added host-lane coverage for `peacock.affectAgentsWindow`: `agentsPanel.border` and `agentsGradient.tintColor` are set/unset with the setting enabled/disabled, both track the accent color for light and dark accents alike, the colors are mirrored to (and cleared from) the user/global settings alongside the workspace settings, and stale `agents.background`/`agentsPanel.background`/`agentsPanel.foreground`/`inactiveSessionView.*` values left over from an earlier version of this feature are cleared from both the workspace and user settings — mirroring the existing `windowBorder` test pattern in `affected-elements.test.ts`
