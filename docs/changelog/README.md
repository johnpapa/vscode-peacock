...
- Added a visual color picker to the **Peacock: Change to a Favorite Color** flow. The favorites Quick Pick now ends with a `$(paintcan) Custom color…` item that opens a webview with a native color well kept in sync with a hex text field. Every edit previews live (the same way hovering a favorite already does), **Apply** keeps the color, and **Cancel** (or closing the panel) reverts to the color that was active before the picker opened. The Quick Pick now always opens, even with zero favorites saved, so the picker is reachable regardless ([#708](https://github.com/johnpapa/vscode-peacock/issues/708))
- Added a standalone **Peacock: Pick a Custom Color (Visual Picker)** command so the picker is directly discoverable from the Command Palette, instead of only being reachable through the Favorites flow. It opens the exact same picker panel/logic as **Custom color…** ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
- Redesigned the color picker panel with a modern layout: a labeled color-well/hex-field group, an **eyedropper button** (shown when the browser EyeDropper API is available) to sample any color from anywhere on screen, and a live **title bar contrast preview** — a sample swatch plus a WCAG contrast ratio badge — that reuses Peacock's own title-bar contrast logic, so a light custom color (e.g. orange or pink) is confirmed to get the same automatic dark-font contrast treatment as any other color, before you even apply it ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
...
- Added a **pick visually** button to **Peacock: Enter a Color**'s input box, opening the same [Custom Color Picker](../guide/README.md#custom-color-picker) (including the eyedropper) as an alternative to typing a hex/named/RGB/HSL/HSV value. `promptForColor()` (`src/inputs.ts`) now uses the `InputBox` API (rather than `showInputBox`) so the button can be attached; typing and pressing Enter/Esc behaves exactly as before ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
...
- Added host-lane coverage for the standalone **Pick a Custom Color (Visual Picker)** command (apply-and-persist, cancel-reverts) and unit coverage for the redesigned panel's eyedropper button and contrast preview markup ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
...
- Added host-lane coverage for the **Enter a Color** pick-visually button: clicking it hands off to the visual picker and the applied color is persisted the same as typed input, canceling the picker leaves the previous color untouched, and every existing hex/named/RGB/HSL/HSV `Enter a Color` format test now drives the real `InputBox` API instead of the retired `showInputBox` stub. Extracted `createFakeInputBox()`/`createFakeWebviewPanel()` test doubles into `src/test/suite/lib/` so `affected-elements.test.ts`, `save-starter-favorite-colors.test.ts`, `color-input.test.ts`, and `custom-color-picker.test.ts` share one implementation ([#708](https://github.com/johnpapa/vscode-peacock/issues/708) follow-up)
