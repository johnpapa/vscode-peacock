...
| peacock.affectAgentsWindow          | Specifies whether Peacock should affect the Agents Window title bar (`agents.background`), while keeping the center session view neutral (`inactiveSessionView.background`/`inactiveSessionView.foreground`). Defaults to false. Available in VS Code 1.120+.                                                    |
...
- `agents.background` — the window shell and title bar, so the Agents Window's title bar matches the accent color the same way `titleBar.activeBackground` does for the standard editor window

VS Code registers `agents.background` as the *default* color for a few other tokens too, including `inactiveSessionView.background` — the background of the large session/composer view in the center of the window. Left alone, that means accenting the title bar would also wash the accent color over that big content area, which breaks Peacock's "chrome strips only" philosophy (the same way Peacock never touches `editor.background` or `sideBar.background` in the standard editor window). To prevent that bleed, Peacock also explicitly sets `inactiveSessionView.background`/`inactiveSessionView.foreground` to a neutral, theme-aware value (not the accent color) whenever `peacock.affectAgentsWindow` is enabled, so the center pane stays visually neutral while only the title bar picks up the accent. `inactiveSessionView.foreground` is computed for contrast against that neutral background (unless `peacock.keepForegroundColor` is enabled). Peacock deliberately does **not** set `agentsPanel.*` — those tokens back the chat/files/terminal card panels and the active session view, and leaving them unset lets those panels keep their own theme-correct default colors.
...
**Colored vertical line between the side bar and editor with modernUI on:** modernUI frames the editor area as a distinct "card" and draws a real, persistent border around it using the `editorGroup.border` color token. Classic layout never renders a visible border there (it uses a subtle shadow instead), so if you have Peacock's **Affect Editor Group Border** setting (`peacock.affectEditorGroupBorder`, off by default) enabled, its color only becomes visible once modernUI is on. This is expected: the setting was already colorizing that token, modernUI just newly renders it. Disable **Affect Editor Group Border** if you don't want that line. See [#652](https://github.com/johnpapa/vscode-peacock/issues/652).
