...
| peacock.affectAgentsWindow          | Specifies whether Peacock should affect the Agents Window's card panel borders (`agentsPanel.border`) and shell gradient tint (`agentsGradient.tintColor`). Defaults to false. Available in VS Code 1.120+.                                                    |
...
- `agentsPanel.border` — the border color of the card panels (chat, files, terminal) in the Agents Window. VS Code registers `agentsCard.border` and `agentsBottomPanel.border` to default onto `agentsPanel.border`, so setting it alone also colors those.
- `agentsGradient.tintColor` — the tint color used in the Agents Window shell's soft background gradient (rendered at low opacity, only in the margins between floating panels).

Peacock deliberately does **not** set `agents.background`, even though it's the token that drives the Agents Window's title bar. VS Code ties `agents.background` unconditionally to three things at once: the title bar, the whole window shell background, and — via a hardcoded rule in VS Code's own stylesheet (`.part.sidebar { background: var(--vscode-agents-background) }`) — the left Sessions sidebar too. There's no separate token that lets Peacock color just the title bar without also painting the sidebar, so — consistent with Peacock never touching `sideBar.background` in the standard editor window — `agents.background` is left alone. For the same reason, Peacock also leaves `agentsPanel.background`/`agentsPanel.foreground` unset: they default onto `activeSessionView.background`, the large session/composer view in the center of the window, and setting them would wash the accent color over that big content area too.
