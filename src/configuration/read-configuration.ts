...
export function prepareColors(backgroundHex: string) {
  ...
  const agentsWindowSettings = collectAgentsWindowSettings(backgroundHex);
...
function collectAgentsWindowSettings(backgroundHex: string) {
  const agentsWindowSettings = {} as ISettingsIndexer;

  if (isAffectedSettingSelected(AffectedSettings.AgentsWindow)) {
    // `agents.background` is a single token shared, unconditionally, by the
    // Agents Window's title bar (titlebarPart.ts), its whole shell
    // background, AND its left Sessions sidebar - VS Code's own CSS sets
    // `.part.sidebar { background: var(--vscode-agents-background); }`
    // (workbench.css) with no separate token to override just the sidebar.
    // There is no way to color the title bar without also painting the
    // sidebar, so - consistent with Peacock never setting `sideBar.background`
    // for the standard window - `agents.background` is intentionally left
    // unset. `agentsPanel.background`/`.foreground` are likewise left unset,
    // since they default onto `activeSessionView.background` (the big
    // session/composer view) and would recolor that content area too.
    //
    // Instead, Peacock colors two narrower, decoupled tokens that can never
    // resolve to a full panel/sidebar background fill:
    //  - `agentsPanel.border`: the border color of the card panels (chat,
    //    files, terminal). `agentsCard.border` and `agentsBottomPanel.border`
    //    both default to this value, so setting it alone colors all three.
    //  - `agentsGradient.tintColor`: a soft, low-opacity radial gradient
    //    tint blended into the shell background - visible only in the
    //    margins between floating panels, never a solid fill.
    const agentsWindowStyle = getElementStyle(backgroundHex);
    agentsWindowSettings[ColorSettings.agentsPanel_border] = agentsWindowStyle.backgroundHex;
    agentsWindowSettings[ColorSettings.agentsGradient_tintColor] = agentsWindowStyle.backgroundHex;
  }
