...
- **A webview panel can be closed three ways: Cancel, Apply, or the tab's own close button (`onDidDispose`).** All three must revert/resolve through the _same_ code path (ideally the same serialized message queue used for regular messages -- see `serializeMessageHandler()` in `color-picker-html.ts`), not a separate direct call. Routing `onDidDispose`'s revert around the queue lets it race a still-in-flight message (e.g. a rapid color-well drag) and either leave a stale value applied or run before/after the wrong thing. Even with that, once the outcome is decided (`settled`), every _other_ handler that mutates external state (e.g. a live-preview callback calling `applyColor()`) must check that flag too and no-op if it's already true -- otherwise a preview message that happens to be queued behind the revert can still overwrite it, since "enqueued after" only guarantees run-order, not that an earlier handler bothered to check whether its result still matters.
...
- **Before marking any PR that adds or touches a webview as ready to merge, do one dedicated adversarial pass over it -- separate from the feature-specific testing that prompted the change.** Treat every value that reaches the webview's HTML or script as attacker/workspace-controlled, not just "a color the user picked," and walk the panel's full lifecycle (hide/show, all 3 ways to close it, an in-flight async message racing any of those) rather than only the happy path a specific ask was testing. Iterative, feature-driven work (implement X, fix Y) does not surface this class of bug on its own -- it takes a deliberate, separate pass with "find problems" as the explicit goal. (This checklist itself was written only after a maintainer-requested review found 9 issues, including a script-injection bug, that several prior feature-focused passes over this same panel had all missed.)
